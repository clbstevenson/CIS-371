...
function compare_rows(row_a, row_b) {
    ...
    if(sorted || prev_col != col_num) {
        var comp_val = lname_a.localeCompare(lname_b);
        if(debug) 
            console.log("\'" + lname_a + "\'.localCompare(\'" + lname_b+"\') = " + comp_val);
    } else {
        // otherwise, already sorted so do a REVERSE sort
        var comp_val = lname_b.localeCompare(lname_a);
        if(debug)
            console.log("\'" + lname_b + "\'.localCompare(\'" + lname_a+"\') = " + comp_val);
    }
    return comp_val;
}
...
function sort(tableID, tableColName, tableCol) {
    ...
    console.log("setting sorted to " + sorted);
